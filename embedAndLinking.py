...
def embedAndLinking(cursor, schema, tables, tNo, relax, pks, work, adj2, relations):
    ...
    for t in tables:
        ...
        mykeys = [desc[0] for desc in cursor.description]           # get coln_names    
        myresults = cursor.fetchall()                               # get all tuples
        dicted_myresult = [dict(zip(mykeys, myresult)) for myresult in myresults]   # map columns to tupes and convert to dictionary
            ...
            if (work[i][j] == 'l') : 
                ...
                for ngh in adj2[tables[i]] :
                    if tNo[ngh] == j :
                        f = True
                        break
                if f == True :          # if already linked then leave
                    work[i][j] = '-'
...
def embed2(t1, t2, pk1, pk2, data, t1Name, t2Name, relations):
    ...
    #checks if FK of t1Name is in t2Name or not
    refersvia = []                              # stores the foreign keys of 1 relation from 2 reln
    ...
    for tt in t1 :
        tt[t2Name] = []     # create a new key in parent table for embedding
    ...
    for t in t2 :
        for tt in t1 :
            ...
            if(flag == True) :          # true if FK of 1 table is PK in another table
                tt[t2Name].append(t)    # embedds
...
def embed1(t1, t2, pk1, pk2, data, t1Name, t2Name, relations):
    ...
    for t in t1 :
        common_attributes = []
        for pk in pk1 :
            common_attributes.append(t[pk])
        lst = []
        for tt in t2 :
            flag = True
            for i in range(min(len(refersvia),  len(common_attributes)) ) :
                if tt[refersvia[i]] != common_attributes[i] :
                    flag = False
...
# function for many to many relationship linking
